...
const colStatementData = 'statementData';
const colStatements = 'statements';
const fldUserId = 'userId';
const fldNotes = 'notes';
const fldIsStarred = 'isStarred';

class FirestoreRepository {
  static UserInfo _userInfo = UserInfo();
  static List<CurrencyRate> _currencyRates = <CurrencyRate>[];
  static List<Statement> _statements = <Statement>[];

  static UserInfo get userInfo => _userInfo;

  static List<CurrencyRate> get currencyRates => _currencyRates;

  static List<Statement> get statements => _statements;

  static Future<void> initDataManager() async {
    await updateData();
  }
  ...
  static Future<List<CurrencyRate>> getCurrencyRates() async {
    ...
    // print(response.data);
    final List<CurrencyRate> currencyRates = <CurrencyRate>[];
    for (final dynamic element in response) {
  ...
  static Future<List<QueryDocumentSnapshot<Map<String, dynamic>>>>
    ...
    final String userId = Get.find<FirebaseAuthRepository>().getFirebaseUser()!.uid;
    return (await FirebaseFirestore.instance
            .collection(colStatementData)
            .doc(userId)
            .collection(colStatements)
            .get())
        .docs;
  }
  ...
  static Future<List<Statement>> getStatements() async {
    ...
    var statementData = await getStatementData();

    for (final element in response) {
      final QueryDocumentSnapshot<Map<String, dynamic>>? elementStatementData =
          statementData.firstWhereOrNull((e) => e.id == element['id']);
      final Statement newStatement = Statement(
        ...
        isStarred: elementStatementData == null
            ? false
            : elementStatementData.get(fldIsStarred),
      );
      //TODO: remove placeholder
      newStatement.notes.add(Note(
  ...
  static Future<void> setStarredStatement(
    String statementId,
    bool isStarred,
  ) async {
    final String userId =
        Get.find<FirebaseAuthRepository>().getFirebaseUser()!.uid;
    final DocumentReference document = FirebaseFirestore.instance
        .collection(colStatementData)
        .doc(userId)
        .collection(colStatements)
        .doc(statementId);
    final bool alreadyExists = (await document.get()).exists;
    if (alreadyExists) {
      await document.update({fldIsStarred: isStarred});
    } else {
      await document.set({
        fldIsStarred: isStarred,
      });
    }
  }
}
